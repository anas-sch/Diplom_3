...
class TestMainPage:
    ...
    def test_ingredient_modal_open_close(self, driver, main_page):
        ...
        assert main_page.is_element_visible(MainPageLocators.MODAL_CONTAINER), "Модальное окно не появилось"
        main_page.close_main_modal()
        assert not main_page.is_element_visible(MainPageLocators.MODAL_CONTAINER), "Модальное окно не закрылось"
